...
internal fun DifficultyBackground(
    ...
    Canvas(modifier.browseSharedBounds(if (shared) "difficulty-color:${chart.id}" else null, stable = true, overlayZ = .5f)) {
        val radius = CornerRadius(14.dp.toPx())
        drawRoundRect(if (available) accent.copy(alpha = .13f) else Background, cornerRadius = radius)
        val width = (if (selected) 2.dp else 1.dp).toPx()
        drawRoundRect(
            color = if (available) accent.copy(alpha = if (selected) .95f else .55f) else Muted.copy(alpha = .7f),
            ...
            style = Stroke(width, pathEffect = if (available) null else PathEffect.dashPathEffect(floatArrayOf(7.dp.toPx(), 4.dp.toPx()))),
        )
    }
}
