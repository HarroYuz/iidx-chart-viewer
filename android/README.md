...
- Textage：首次启动由原生 `HttpURLConnection` 获取 `/score/` 声明的同域 `titletbl.js`、`datatbl.js`、`actbl.js`、`cstbl*.js`、`scrlist.js` 等数据表，在 APK 内关联曲目元数据、难度、NOTE 数和 BPM；后续启动重新检查这些脚本。用户点进具体难度后，才获取谱面页面及同源脚本，并把该难度的谱面 JSON 缓存到应用私有目录。不打开 Textage 页面、不发送外部浏览器 Intent。
