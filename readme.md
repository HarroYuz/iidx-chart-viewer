...
Textage 删除曲在版本右侧显示红字“删除曲”，家用版专属曲目显示蓝字“家用版”，曲名保持普通颜色。筛选区以紧凑按钮多选关键字搜索范围（曲风 / 曲名 / 曲师）和曲目类型（删除曲 / 家用版 / 稼动中），默认三类全选。等级和版本使用可展开的多选网格，分别每行四项、两项；未选时不限，同一维度内取并集，不同维度间取交集。版本按 CS、substream、1～33 代排列，11 代起显示版本号。未知曲目状态仅在三类全选时保留。重置或清除筛选后恢复全部类型、等级和版本。曲目候选优先选可浏览的当前收录版本；成绩历史在 NOTE 数校验通过的候选中优先选当前收录版本。旧版可能缺少对应 BJM 成绩或雷达；无可用雷达时显示“暂无雷达数据”。
